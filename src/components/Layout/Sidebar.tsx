...
export function Sidebar({ activeCategory, setActiveCategory, onClose }: SidebarProps) {
  ...
  const categoryIcons = [
    ...
    { id: "chicken-feet", icon: <Drumstick size={20} className="text-red-500" /> },
  ...
  const getNavLinkClasses = (isActive: boolean) => {
    return `w-full flex items-center p-3 rounded-md transition-colors ${isActive
    ...
    <div className="w-full h-full bg-white p-4 rounded-lg shadow-sm">
          ...
          <div className="space-y-2">
            {categories.map((category) => (
              <button
                ...
                className={`w-full flex items-center p-3 rounded-md transition-colors ${activeCategory === category.id
      ...
      <div className={`${isHomePage ? 'md:hidden mt-6 pt-6 border-t border-gray-200' : ''}`}>
        ...
        <Link href="/"
          ...
          onClick={onClose}
        >
          <span className="mr-3"><Coffee size={20} className="text-gray-500" /></span>
          <span>Món ăn</span>
        </Link>
        ...
        <Link href="/about"
          ...
          onClick={onClose}
        >
          <span className="mr-3"><Info size={20} className="text-gray-500" /></span>
        ...
        <Link href="/contact"
          ...
          onClick={onClose}
        >
          <span className="mr-3"><Phone size={20} className="text-gray-500" /></span>
          <span>Liên hệ</span>
        </Link>
      </div>
    </div>
